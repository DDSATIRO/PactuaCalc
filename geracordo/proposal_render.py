...
class ProposalPdfLayout:
    ...
    def callout(
        ...
        content_height = 14 + (len(title_lines) * 14) + (len(body_lines) * 12) + (len(accent_lines) * 16) + (14 if bottom_prefix else 0) + 10
        ...
        if bottom_prefix and bottom_accent:
            ...
            prefix_w = self.estimate_width(bottom_prefix, 11)
            self.pdf.text(MARGIN_X + 8 + prefix_w, line_y, bottom_accent, size=11, font="F2", color=(0.76, 0.08, 0.08))
            line_y -= 14
        self.cursor_y = y - 10
